class Obstaculo extends Personaje {
    constructor(personaje, tiempo, escenario, recolectable) {
        super(personaje)
        this.tiempoEspera = tiempo;
        this.esRecolectable = recolectable;
        if (escenario == 1)
    ...
    init() {
        if (this.esRecolectable) {
            let tiempo = setInterval(() => {
                console.log("movilidad diamante")
                this.div.classList.remove('hide');
                this.div.classList.add('movilidad-diamante');
            }, this.tiempoEspera);
        } else {
        setTimeout(() => {
            this.div.classList.remove('hide');
